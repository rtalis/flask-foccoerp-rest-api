...
def test_search_item(client: FlaskClient):
    response = client.get('/search_item', query_string={'descricao': 'example'})
    assert response.status_code == 200
    assert isinstance(response.json, list)

def test_search_item_fuzzy(client: FlaskClient):
    response = client.get('/search_item_fuzzy', query_string={'descricao': 'exmple'})
    assert response.status_code == 200
    assert isinstance(response.json, list)

def test_search_item_id(client: FlaskClient):
    response = client.get('/search_item_id', query_string={'item_id': '123'})
    assert response.status_code == 200
    assert isinstance(response.json, list)

def test_search_cod_pedc(client: FlaskClient):
    response = client.get('/search_cod_pedc', query_string={'cod_pedc': '456'})
    assert response.status_code == 200
    assert isinstance(response.json, list)