...
def get_nfe_by_purchase():
    ...
    from app.models import Supplier, NFEData, PurchaseOrder, NFEEmitente, Company, PurchaseItemNFEMatch
    ...
    if purchase_order.fornecedor_id:
        supplier = Supplier.query.filter(
            or_(Supplier.cod_for == str(purchase_order.fornecedor_id), Supplier.id_for == purchase_order.fornecedor_id)
        ).first()
    else:
    ...
    # Get the Company CNPJ for destination filtering
    company = Company.query.filter_by(cod_emp1=str(purchase_order.cod_emp1)).first()
    company_cnpj = ''.join(filter(str.isdigit, str(company.cnpj))) if company and company.cnpj else ""

    start_date = purchase_order.dt_emis - relativedelta(days=30)
    end_date = datetime.now() 

    nfe_data = []
    local_chaves = set()
    cnpj_root = fornecedor_cnpj[:8]
    fornecedor_nome = purchase_order.fornecedor_descricao

    # --- 1. Query Local Database ---
    try:       
        local_nfes = NFEData.query.join(NFEData.emitente).filter(
            and_(
                NFEData.data_emissao >= start_date,
                NFEData.data_emissao <= end_date,
                or_(
                    NFEEmitente.cnpj.startswith(cnpj_root),
                    NFEEmitente.nome.startswith(fornecedor_nome)
                )
            )
        ).order_by(NFEData.data_emissao.desc()).all()

        for nfe in local_nfes:
            local_chaves.add(nfe.chave)
            nfe_data.append({
                'chave': nfe.chave,
                'numero': nfe.numero or '',
                'data_emissao': nfe.data_emissao.isoformat() if nfe.data_emissao else '',
                'fornecedor': nfe.emitente.nome if nfe.emitente else '',
                'valor': str(nfe.valor_total) if nfe.valor_total is not None else '',
                'xml_content': nfe.xml_content,
                'stored_in_database': True,
                'source': 'local_db'
            })
    except Exception as e:
        print(f"Error querying local database: {str(e)}")

    # --- 2. Query SIEG API (Modern V1 ZIP Endpoint) ---
    sieg_start = purchase_order.dt_emis - relativedelta(days=15)
    sieg_end = sieg_start + timedelta(days=44)
    ...
    sieg_payload = {
        ...
        "Take": 50,
        "Skip": 0,
        "DataEmissaoInicio": sieg_start.strftime('%Y-%m-%d'),
        "DataEmissaoFim": sieg_end.strftime('%Y-%m-%d'),
        "CnpjEmit": fornecedor_cnpj,
        "BaixarEventos": True 
    }
    
    if company_cnpj:
        sieg_payload["CnpjDest"] = company_cnpj

    try:
        response = requests.post(
            'https://api.sieg.com/api/v1/baixar-xmls',
            json=sieg_payload,
            headers=headers,
            timeout=20
        )
        ...
        if response.status_code == 200 and response.content:
            try:
                with zipfile.ZipFile(io.BytesIO(response.content)) as z:
                    ...
                    for name in xml_names:
                        with z.open(name) as f:
                            ...
                            # Filter out events (Cancellations/CC-e) since we only want Invoices to match against the PO
                            ...
                            # Skip if we already pulled it from the local DB
                            if chave_acesso in local_chaves:
                                continue
                            
                            # Instantly save the missing XML to the database
                            parse_and_store_nfe_xml(xml_content)
                ...
                print("SIEG returned an invalid ZIP file.")
        elif response.status_code == 429:
            print("Rate limited by SIEG during live PO search.")
        elif response.status_code not in (404, 400):
            print(f"SIEG API Error: {response.status_code} - {response.text}")

    except Exception as e:
        print(f"Error querying SIEG V1 API: {str(e)}")
    
    # If no data found in both local DB and SIEG API
    if not nfe_data:
        return jsonify({'message': 'No NFE data found for this purchase order'}), 404
    
    # --- 3. Add match scores for the UI Drawer ---
    po_matches = PurchaseItemNFEMatch.query.filter_by(
        cod_pedc=cod_pedc,
    ...
    score_map = {}
    for m in po_matches:
        if m.nfe_chave not in score_map or m.match_score > score_map[m.nfe_chave]:
            score_map[m.nfe_chave] = m.match_score
...
def search_nfe():
    ...
    """
    from app.models import NFEData, NFEEmitente, NFEDestinatario, NFEItem, NFEntry, PurchaseOrder, PurchaseItem, PurchaseItemNFEMatch, Company, NFEEvento
    from datetime import datetime
    from fuzzywuzzy import fuzz
    from sqlalchemy import and_, or_, func
    from app.utils import normalize_uom
    ...
    # Extract the filter for document types
    ...
    def _status_from_diff(diff_pct, green_max=5, yellow_max=20):
        if diff_pct is None:
            return 'gray'
        if diff_pct < green_max:
            return 'green'
        if diff_pct < yellow_max:
            return 'yellow'
        return 'red'
    ...
    def compute_item_status(po_item, nfe_item_preco, nfe_item_qty, nfe_item_id=None,
        ...
        po_price = float(po_item.preco_unitario) if po_item.preco_unitario is not None else None
        po_qty = float(po_item.quantidade) if po_item.quantidade is not None else None
        po_uom_clean = normalize_uom(po_item.unidade_medida) if po_item.unidade_medida else ''
        nfe_uom_clean = normalize_uom(nfe_uom) if nfe_uom else ''
        ...
        if converted:
            ...
            result['unit_price']['status'] = _status_from_diff(unit_diff)
        result['unit_price']['diff_pct'] = round(unit_diff, 2) if unit_diff is not None else None
        result['unit_price']['converted'] = converted

        po_line_total = po_price * po_qty if po_price is not None and po_qty is not None else None
        nfe_line_total = (
            float(nfe_item_preco) * float(nfe_item_qty)
            if nfe_item_preco is not None and nfe_item_qty is not None
            else None
        )
        ...
        for nfe in nfes:
                                ...
                                if not already_linked:
                                    ...
                                    item_status = compute_item_status(
                                        ...
                                        nfe_item_preco=match.nfe_item_preco if match.nfe_item_preco is not None else (
                                            nfe_item.valor_unitario_comercial if nfe_item else None
                                        ),
                                        nfe_item_qty=match.nfe_item_quantidade if match.nfe_item_quantidade is not None else (
                                            nfe_item.quantidade_comercial if nfe_item else None
                                        ),
            ...
            nfe_results.append({
                ...
                'eventos': [{
                    ...
                    'data_evento': evt.data_evento.isoformat() if evt.data_evento else None,
                } for evt in nfe.eventos] if hasattr(nfe, 'eventos') and nfe.eventos else []
            })
