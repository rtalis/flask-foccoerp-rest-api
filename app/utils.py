def parse_xml(xml_data):
    ...
    for order in root.findall('.//TPED_COMPRA'):
        ...
        order_data = {
            ...
            'total_liquido_ipi': float(order.find('TOT_LIQUIDO_IPI1').text.replace(',', '.')) if order.find('TOT_LIQUIDO_IPI1') is not None and order.find('TOT_LIQUIDO_IPI1').text else None,
