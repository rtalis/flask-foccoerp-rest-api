...
def upgrade():
    # Use conditional drops to avoid failing if indexes don't exist
    try:
        op.execute("DROP INDEX IF EXISTS ix_purchase_nfe_match_score;")
    except Exception:
        pass
    try:
        op.execute("DROP INDEX IF EXISTS ix_purchase_nfe_matches_cod_emp1;")
    except Exception:
        pass
    try:
        op.execute("DROP INDEX IF EXISTS ix_purchase_nfe_matches_cod_pedc;")
    except Exception:
        pass
    try:
        op.execute("DROP INDEX IF EXISTS ix_purchase_nfe_matches_nfe_chave;")
    except Exception:
        pass

    # Drop table if exists (swallow errors to allow upgrade to continue)
    try:
