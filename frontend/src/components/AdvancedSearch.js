...
const AdvancedSearch = ({ onLogout }) => {
  ...
  const handleSearch = async (nextPage = 1) => {
    ...
    try {
      ...
      // Ignore out-of-order responses from older searches
      ...
      setTotalResults(
        usingEnhanced
          ? response.data?.total_results || purchases.length
          : purchases.length
      );
    } catch (err) {
        ...
        <Toolbar>
          ...
          <Box sx={{ flexGrow: 1 }}>
            ...
            <Typography variant="body2" color="text.secondary">
              Combine termos, filtros e valores para localizar itens
              rapidamente.
            </Typography>
          </Box>
          <Button
          ...
          <Grid item xs={12}>
            <Paper elevation={1} sx={{ p: 2 }}>
              ...
              {loading ? (
                ...
                <Typography variant="body2" color="text.secondary">
                  Informe um termo e clique em buscar para visualizar os
                  resultados.
                </Typography>
              ) : (
                <Stack spacing={2}>
                  {results.map((purchase) => (
                    <Card key={purchase.order.cod_pedc} variant="outlined">
                      <CardHeader
                        title={`${purchase.order.cod_pedc} — ${
                          purchase.order.fornecedor_descricao ||
                          "Fornecedor não informado"
                        }`}
                        subheader={`Emitido em ${
                          purchase.order.dt_emis || "—"
                        } · Comprador: ${purchase.order.func_nome || "—"}`}
                        action={
                          <Stack
                            direction="row"
                            spacing={1}
                          ...
                          >
                            ...
                            <Typography variant="h6" color="primary">
                              {new Intl.NumberFormat("pt-BR", {
                                ...
                                purchase.order.adjusted_total ||
                                  purchase.order.total_pedido_com_ipi ||
                                  0
                              )}
                            </Typography>
                          </Stack>
